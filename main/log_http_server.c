...
#define RECENT_REBOOT_WINDOW_MS		180000U
                                   ...
                                   size_t topology_len)
{
	...
	uint16_t diag_flags = topology->diag_flags;
	uint16_t reset_reason = 0;
	uint32_t boot_seq = 0;
	uint16_t parent_disconnect_count = 0;
	uint16_t no_parent_count = 0;
	...
	if (has_diag) {
		const mesh_v2_topology_v3_payload_t *v3 =
			(const mesh_v2_topology_v3_payload_t *)topology;
		diag_flags = v3->v2.base.diag_flags;
		reset_reason = v3->reset_reason;
		boot_seq = v3->boot_seq;
		parent_disconnect_count = v3->parent_disconnect_count;
		no_parent_count = v3->no_parent_count;
		rootless_count = v3->rootless_count;
		soft_reconnect_count = v3->soft_reconnect_count;
		mesh_restart_count = v3->mesh_restart_count;
	...
	portENTER_CRITICAL(&s_nodes_lock);
	{
		for (uint32_t i = 0; i < s_nodes_count; i++) {
			if (mac_eq(s_nodes[i].mac, mac)) {
				...
				s_nodes[i].last_send_err = last_mesh_send_err;
				s_nodes[i].recovery_phase = topology->recovery_phase;
				s_nodes[i].log_stream_enabled = topology->log_stream_enabled != 0;
				s_nodes[i].diag_valid = has_diag;
				s_nodes[i].diag_flags = diag_flags;
				s_nodes[i].reset_reason = reset_reason;
				s_nodes[i].boot_seq = boot_seq;
				s_nodes[i].parent_disconnect_count = parent_disconnect_count;
				s_nodes[i].no_parent_count = no_parent_count;
				...
				if ((diag_flags & MESH_V2_TOPO_DIAG_RECENT_REBOOT) ||
				    (s_nodes[i].uptime_valid &&
				     topology->uptime_s + 10U < s_nodes[i].uptime_s)) {
					s_nodes[i].recent_reboot_seen_ms = now;
				}
				if (has_ota_slots) {
		...
		if (!recorded && s_nodes_count < LOG_HTTP_MAX_NODES) {
			...
			ent->last_send_err = last_mesh_send_err;
			ent->recovery_phase = topology->recovery_phase;
			ent->log_stream_enabled = topology->log_stream_enabled != 0;
			ent->diag_valid = has_diag;
			ent->diag_flags = diag_flags;
			ent->reset_reason = reset_reason;
			ent->boot_seq = boot_seq;
			ent->parent_disconnect_count = parent_disconnect_count;
			ent->no_parent_count = no_parent_count;
                                               ...
                                               uint32_t now)
{
	if (!node || node->topology_seen_ms == 0) {
		return append_fmt(out, cap, pos,
		                  ...
		                  "\"diag_valid\":false,\"diag_flags\":0,"
		                  "\"reset_reason\":0,\"boot_seq\":0,"
...
static esp_err_t http_root_get(httpd_req_t *req)
{
	static const char html[] =
		...
		"  const keep=new Set(desired.map(n=>n.mac));\n"
		"  for(const o of Array.from(s.options)){if(!keep.has(o.value))o.remove()}\n"
		"  for(const n of desired){const o=existing.get(n.mac);if(o)s.appendChild(o)}\n"
		"}\n"
		...
		"  if(!selectionBusy&&remembered&&remembered!==serverCur&&nodeInList(nodes,remembered)){cur=remembered;if(!reselectBusy)setTimeout(()=>reselectNode(remembered),0)}\n"
		"  else if(!selectionBusy&&rememberedMissing){cur=remembered}\n"
		"  const key=nodeListKey(nodes,cur,rememberedMissing);const focused=document.activeElement===s&&!force;if(key===lastNodes&&!pendingNodeSync)return;if(focused&&key===pendingNodeKey)return;\n"
		"  selectedMac=cur||selectedMac;syncNodeOptions(s,nodes,rememberedMissing,remembered,rememberedTag,!focused);\n"
		"  if(focused){pendingNodeSync={j:j,raw:raw};pendingNodeKey=key}else{pendingNodeSync=null;pendingNodeKey='';lastNodes=key}\n"
		...
		"  if(!j)return;\n"
		"  if(mac!==selectedMac||(j.mac&&String(j.mac).toLowerCase()!==selectedMac))return;\n"
		...
		"  onNodeSel();\n"
		"});\n"
		"document.getElementById('nodeSel').addEventListener('blur',flushNodeOptions);\n"
		...
		"function taskLinkMac(hash){const match=/^#tasks=([0-9a-f]{12})$/i.exec(hash||'');return match?match[1].toLowerCase():''}\n"
		"const taskLinkTarget=taskLinkMac(location.hash);if(taskLinkTarget){selectedMac=taskLinkTarget;rememberNode(taskLinkTarget,cachedNodeTag(taskLinkTarget)||'node');tasksVisible=true;document.body.classList.add('showTasks');document.getElementById('tm').textContent='ON';setTopActive('tasksBtn',true);clearTasksPanel()}\n"
		"updateAdminUi();\n"
		...
		"loadNodes(true).finally(()=>loadUiStatus(!!taskLinkTarget).finally(()=>{startUiStream();setTimeout(tick,350)}));\n"
		"</script>\n"
		"</body></html>\n";
